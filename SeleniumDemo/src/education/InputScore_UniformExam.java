/*
 * 这个代码是录入统考成绩的自动化代码，可以一键录入所有年级班级科目所有统考的考试成绩，分数段都会比较合理
 * 只需定位到代码的102行修改录入时间即可
...
public class InputScore_UniformExam {
	
	public static WebDriver driver = null;
	//教育后台地址，如果有改变请修改URL地址
	public static final String URL = "http://192.168.11.136:8082/edu/login.html";
	//用户名，如果有变动请修改
	public static final String USERNAME = "18872573200";
	//用户密码
        ...
        for (int i = 0; i < gradeOptions.size(); i++) {
        	...
        	for (int j = 0; j < classOptions.size(); j++) {
        		...
        		for (int j2 = 0; j2 < subjectOptions.size(); j2++) {
        			...
        			//点击统考checkbox 如果切换科目的时候，checkbox被勾选，那么就不用再次点击，如果没有勾选就点击勾选
        			WebElement uniformExam = driver.findElement(By.id("score_isUniform"));
        			if(uniformExam.isSelected()==false){
        			uniformExam.click();
        			waitForPageToLoad(500);
        			}
					...
					//通过JS设置时间
					String js="document.getElementById('score_examDate').removeAttribute('readonly');document.getElementById('score_examDate').setAttribute('value','2013-06-20);";
